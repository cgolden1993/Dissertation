...
def get_forecast(zip):
    ...
    forecast_url = forecast_api + config.api_key + '/' + str(geopoint[0]) + ',' + str(geopoint[1]) + '?units=si&exclude=currently,minutely,daily,alerts,flags&extend=hourly'

    #extract info from json and grab the hourly forecast
    forecast_json = requests.get(forecast_url).json()
    hourly_forecast = forecast_json['hourly']
